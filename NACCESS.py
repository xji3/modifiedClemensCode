...
def run_naccess_subpro(pdb_file_dir, pdb_id, probe_size = None, z_slice = None,
                naccess = 'naccess', temp_path = '/tmp/'):

    # run Naccess for the pdb_file and save output in the same directory
    pdb_file=pdb_file_dir + pdb_id + '.pdb'
    command = [naccess, pdb_file]
    if probe_size:
        command.append('-p')
        command.append(probe_size)
    if z_slice:
        command.append('-z')
        command.append(z_slice)

    if os.path.isfile(pdb_file + '.gz'):
        ...
        print subprocess.check_output(command,cwd=pdb_file_dir)
    ...
    rsa_file = pdb_file_dir + pdb_id + '.rsa'
    rf = open(rsa_file)
    rsa_data = rf.readlines()
    rf.close()
    asa_file = pdb_file_dir + pdb_id + '.asa'
    af = open(asa_file)
    asa_data = af.readlines()
    af.close()
    return rsa_data, asa_data
...
class NACCESS(AbstractResiduePropertyMap):
    ...
    def __init__(self, chain, pdb_file_dir = None, pdb_id = None,
                 naccess_binary = 'naccess', tmp_directory = '/tmp'):
        res_data, atm_data = run_naccess_subpro(pdb_file_dir, pdb_id, naccess = naccess_binary,
...
class NACCESS_atomic(AbstractAtomPropertyMap):
    ...
    def __init__(self, model, pdb_file_dir = None, pdb_id = None,
                 naccess_binary = 'naccess', tmp_directory = '/tmp'):
        res_data, atm_data = run_naccess_subpro(pdb_file_dir, pdb_id, naccess = naccess_binary,
                                         temp_path = tmp_directory)
